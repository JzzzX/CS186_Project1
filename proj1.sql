...
-- Question 4ii
CREATE VIEW bins_statistics(bin_start, bin_end, bin_width)
AS
  SELECT min_salary AS bin_start, 
         max_salary AS bin_end, 
         CAST(((max_salary - min_salary) / 10) AS INT) AS bin_width
  FROM q4i
  WHERE yearid = 2016
;
  
CREATE VIEW bins(binid, bin_start, bin_width)
AS
  SELECT CAST((salary / bin_width) AS INT), bin_start, bin_width
